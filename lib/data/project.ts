...
export async function getProjectsByUsername(
  ...
  return withErrorHandling(async () => {
    ...
    if (!usernameValidation.success) {
      throw new Error(usernameValidation.error.errors[0]?.message || "Invalid username");
    }
    ...
    if (!paginationValidation.success) {
      throw new Error(paginationValidation.error.errors[0]?.message || "Invalid pagination parameters");
    }
 ...
 */
export const getProjectByUsernameAndSlug = dedupe(async (
  username: string,
  projectSlug: string
  ...
  return withErrorHandling(async () => {
    ...
    if (!slugValidation.success) {
      throw new Error(slugValidation.error.errors[0]?.message || "Invalid project slug");
    }
...
export async function getAllProjects(
  ...
  return withErrorHandling(async () => {
    ...
    if (!userValidation.success) {
      throw new Error(userValidation.error.errors[0]?.message || "Invalid user ID");
    }
...
): Promise<DataResponse<PaginatedResponse<{
  project: Project;
  featuredImage: Media | null;
}>>> {
  return withErrorHandling(async () => {
 ...
 */
export const getProjectCount = dedupe(async (
  userId: string
