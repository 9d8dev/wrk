...
import { profile, socialLink, user, media } from "@/db/schema";
...
import type { Profile, SocialLink, User, Media } from "@/db/schema";
 ...
 */
export const getProfileByUserId = dedupe(async (
  userId: string
): Promise<DataResponse<Profile | null>> => {
  return withErrorHandling(async () => {
    // Validate input
 ...
 */
export const getSocialLinksByProfileId = dedupe(async (
  profileId: string
): Promise<DataResponse<SocialLink[]>> => {
  return withErrorHandling(async () => {
    // Validate input
    const validation = profileIdSchema.safeParse(profileId);
    if (!validation.success) {
      throw new Error(validation.error.errors[0]?.message || "Invalid profile ID");
    }
 ...
 */
export const getProfileWithUser = dedupe(async (
  profileId: string
 ...
 */
export async function hasCompletedProfile(userId: string): Promise<DataResponse<boolean>> {
  return withErrorHandling(async () => {
    // Validate input
    const validation = userIdSchema.safeParse(userId);
    if (!validation.success) {
 ...
 */
export async function getProfileStats(profileId: string): Promise<DataResponse<{
