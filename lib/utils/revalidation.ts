 ...
 */
export async function forceRevalidateProfile(username: string, userId?: string) {
...
export const RevalidationStrategies = {
  ...
  IMMEDIATE: {
    profile: (username: string, userId?: string) => revalidateUserProfile(username, userId),
    username: (old: string, new_: string, userId: string) => revalidateUsernameChange(old, new_, userId),
  },
  
  // Moderate revalidation for content changes
  CONTENT: {
    projects: (username: string, userId: string) => revalidateUserProjects(username, userId),
    theme: (username: string, userId: string) => revalidateUserTheme(username, userId),
  },
 ...
 */
export async function debugRevalidation(username: string, type: 'profile' | 'projects' | 'theme' = 'profile') {
