...
export function isCustomerEvent(event: PolarWebhookEvent): event is CustomerCreatedEvent | CustomerUpdatedEvent {
  return event.type.startsWith('customer.');
}

export function isOrderEvent(event: PolarWebhookEvent): event is OrderPaidEvent | OrderRefundedEvent {
  return event.type.startsWith('order.');
}

export function isSubscriptionEvent(event: PolarWebhookEvent): event is SubscriptionCreatedEvent | SubscriptionActiveEvent | SubscriptionCanceledEvent | SubscriptionRevokedEvent {
  return event.type.startsWith('subscription.');
}

export function isCheckoutEvent(event: PolarWebhookEvent): event is CheckoutCreatedEvent {
