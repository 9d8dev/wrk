...
console.log(`✓ VERCEL_API_TOKEN: ${VERCEL_API_TOKEN ? '✅ Set' : '❌ Missing'} ${VERCEL_API_TOKEN ? `(${VERCEL_API_TOKEN.length} chars)` : ''}`);
console.log(`✓ VERCEL_PROJECT_ID: ${VERCEL_PROJECT_ID ? '✅ Set' : '❌ Missing'} ${VERCEL_PROJECT_ID ? `(${VERCEL_PROJECT_ID})` : ''}`);
console.log(`✓ VERCEL_TEAM_ID: ${VERCEL_TEAM_ID ? '✅ Set' : '⚠️  Optional'} ${VERCEL_TEAM_ID ? `(${VERCEL_TEAM_ID})` : ''}`);
console.log();
...
async function runTests() {
  ...
  try {
    ...
    if (result.status === 200) {
      ...
      console.log(`Found ${result.data.domains ? result.data.domains.length : 0} existing domains`);
