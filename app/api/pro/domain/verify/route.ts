		...
		try {
			...
			try {
				...
				return {
					...
					pointsToVercel: false,
					error:
						"Domain does not resolve. Please check your DNS configuration.",
...
export async function POST(request: NextRequest) {
	try {
		...
		if (!dnsCheck.resolves || !dnsCheck.pointsToVercel) {
			...
			const newStatus =
				dnsCheck.error && !dnsCheck.error.includes("does not resolve")
					? "error"
					: "pending";
		...
		if (statusResult.success && statusResult.data) {
			...
			if (configured && ssl.state === "READY") {
				...
				return NextResponse.json({
					...
					status: "ssl_pending",
					message:
						"Domain is configured but SSL certificate is still being provisioned. This can take a few minutes.",
					instructions: {
